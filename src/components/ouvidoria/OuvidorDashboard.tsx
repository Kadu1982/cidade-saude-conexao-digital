...
export const OuvidorDashboard = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      'nova': { label: 'Nova', variant: 'default' as const, color: 'bg-blue-100 text-blue-800' },
      'em_andamento': { label: 'Em Andamento', variant: 'secondary' as const, color: 'bg-yellow-100 text-yellow-800' },
      'concluida': { label: 'Concluída', variant: 'outline' as const, color: 'bg-green-100 text-green-800' },
      'atrasada': { label: 'Atrasada', variant: 'destructive' as const, color: 'bg-red-100 text-red-800' }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{estatisticas.totalManifestacoes}</div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium">Novas Manifestações</CardTitle>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium">Manifestações Atrasadas</CardTitle>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round((estatisticas.concluidas / estatisticas.totalManifestacoes) * 100)}%
            </div>
            <p className="text-xs text-muted-foreground">
              {estatisticas.concluidas} de {estatisticas.totalManifestacoes} manifestações
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{estatisticas.satisfacaoMedia.toFixed(1)}/5.0</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{estatisticas.tempoMedioResposta} dias</div>
            <p className="text-xs text-muted-foreground">
              Meta: 5 dias úteis
            </p>
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {Object.entries(estatisticas.manifestacoesPorTipo).map(([tipo, quantidade]) => (
        ...
        <CardContent>
          <div className="space-y-4">
            {manifestacoesRecentes.map((manifestacao) => (
              <div key={manifestacao.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center gap-4">
                  <div className="text-xl">{getTipoEmoji(manifestacao.tipo)}</div>
                  <div>
