    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium">Pesquisas Enviadas</CardTitle>
              <Send className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{estatisticas.totalEnviadas}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{estatisticas.taxaResposta}%</div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {Object.entries(estatisticas.distribuicaoNotas).reverse().map(([nota, quantidade]) => (
              <div key={nota} className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground w-12">
                  {Math.round((quantidade / estatisticas.totalRespondidas) * 100)}%
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        <div className="text-muted-foreground">Data de Envio:</div>
                        <div>{pesquisa.dataEnvio.toLocaleDateString('pt-BR')}</div>
                      </div>
                      <div>
                    ...
                    {pesquisa.comentario && (
                      <div className="mt-3 p-3 bg-muted rounded text-sm">
                        <div className="text-muted-foreground mb-1">Comentário:</div>
                        <div>"{pesquisa.comentario}"</div>
                      </div>
                    )}
                    {pesquisa.dataResposta && (
                      <div className="mt-2 text-xs text-muted-foreground">
                        Respondida em: {pesquisa.dataResposta.toLocaleDateString('pt-BR')}
                      </div>
                    )}
                  </div>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <input type="checkbox" defaultChecked className="rounded" />
                <span className="text-sm">Enviar lembrete após 3 dias sem resposta</span>
              </div>
            </div>
