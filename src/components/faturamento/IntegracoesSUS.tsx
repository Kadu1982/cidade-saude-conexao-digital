...
export const IntegracoesSUS = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge className={variants[status as keyof typeof variants] || variants.desconectado}>
  ...
  const integracoesComErro = INTEGRACOES.filter(i => i.status === "erro").length;
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{integracoesConectadas}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{integracoesComErro}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {Math.round((integracoesConectadas / INTEGRACOES.length) * 100)}%
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">{integracao.url}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right text-xs">
                    <div className="font-medium">Última: {integracao.ultimaAtualizacao}</div>
                    <div className="text-gray-500">Próxima: {integracao.proximaAtualizacao}</div>
                    <div className="text-gray-500">Freq: {integracao.frequencia}</div>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      onClick={() => handleSincronizar(integracao.id, integracao.nome)}
              ...
              {integracao.status === "erro" && (
                <Alert className="mt-3">
                  ...
                  <AlertDescription>
                    <strong>Erro de conexão.</strong> Verifique a conectividade e tente novamente.
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Importante:</strong> As integrações seguem as especificações técnicas oficiais do Ministério da Saúde.
          Problemas de conectividade podem afetar a sincronização de dados.
        </AlertDescription>
      </Alert>
    </div>
  );
