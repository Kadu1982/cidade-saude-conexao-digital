...
export const IAAssistant = () => {
  ...
  const [conversa, setConversa] = useState<Array<{tipo: 'usuario' | 'ia', texto: string}>>([
    {
      tipo: 'ia',
      texto: 'Olá! Sou o assistente de IA do Sistema de Gerenciamento de Saúde. Como posso ajudar você hoje?'
    }
  ]);
  ...
  const simularResposta = (texto: string) => {
    if (texto.includes('agendamento') || texto.includes('agendar') || texto.includes('consulta')) {
    ...
    if (texto.includes('paciente') || texto.includes('cadastro') || texto.includes('cadastrar')) {
    ...
    if (texto.includes('faturamento') || texto.includes('sisus') || texto.includes('sus')) {
    ...
    if (texto.includes('relatório') || texto.includes('relatorio') || texto.includes('estatística')) {
    ...
    if (texto.includes('prontuário') || texto.includes('prontuario') || texto.includes('histórico')) {
      return 'O prontuário eletrônico do paciente está disponível nos módulos de atendimento. Basta pesquisar pelo nome ou número do cartão SUS do paciente para acessar todo o histórico clínico.';
    }
    ...
    <div className="flex flex-col h-[500px]">
          ...
          {conversa.map((msg, index) => (
            <div
              ...
              className={cn(
                ...
                msg.tipo === 'usuario' ? "ml-auto max-w-[80%] bg-primary text-primary-foreground" : "mr-auto max-w-[80%] bg-muted"
              )}
            >
      ...
      <div className="border-t pt-4">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Este assistente de IA está conectado aos módulos do sistema e pode ajudar com dúvidas sobre procedimentos, fluxos de trabalho e funcionalidades.
        </p>
      </div>
    </div>
  );
