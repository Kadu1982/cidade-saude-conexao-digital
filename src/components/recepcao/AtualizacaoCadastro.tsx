...
export const AtualizacaoCadastro: React.FC = () => {
  const [currentStep, setCurrentStep] = useState<'auth' | 'identify' | 'update'>('auth');
    ...
    return (
      <div className="space-y-6">
        <Alert>
          ...
          <AlertDescription>
            <strong>Autorização Necessária:</strong> Para atualizar cadastros é necessário 
            autenticação biométrica do recepcionista autorizado.
          </AlertDescription>
        </Alert>
    ...
    return (
      <div className="space-y-6">
        <Alert>
          <Shield className="h-4 w-4" />
          <AlertDescription>
            <strong>Recepcionista Autenticado:</strong> {authenticatedUser?.nome} - {authenticatedUser?.unidade}
          </AlertDescription>
        </Alert>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <p>{selectedPatient?.dataNascimento ? new Date(selectedPatient.dataNascimento).toLocaleDateString() : 'Não informado'}</p>
            </div>
          </div>
