...
const MOCK_SOLICITACOES: SolicitacaoRegulacao[] = [
  {
    ...
    cid: "I25.9",
    justificativa: "Paciente com dor precordial recorrente e histórico familiar de doença coronariana",
...
export const RegulacaoAmbulatorial = () => {
  ...
  const [solicitacoes, setSolicitacoes] = useState<SolicitacaoRegulacao[]>(MOCK_SOLICITACOES);
  const [filtroStatus, setFiltroStatus] = useState<string>("");
  const [filtroPrioridade, setFiltroPrioridade] = useState<string>("");

  const solicitacoesFiltradas = solicitacoes.filter(sol => {
    return (!filtroStatus || sol.status === filtroStatus) &&
  ...
  const aprovarSolicitacao = (solicitacaoId: string) => {
    ...
    toast({
      ...
      description: "A solicitação foi aprovada e encaminhada para agendamento",
    });
  };
  ...
  const getPrioridadeBadge = (prioridade: SolicitacaoRegulacao['prioridade']) => {
    const variants = {
    ...
    return (
      <Badge className={variants[prioridade]}>
        {labels[prioridade]}
      </Badge>
    );
  };
  ...
  const estatisticas = {
    ...
    emergencias: solicitacoes.filter(s => s.prioridade === 'emergencia').length
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{estatisticas.emergencias}</div>
          ...
          <div className="space-y-4">
            {solicitacoesFiltradas
              .sort((a, b) => {
                ...
                if (b.prioridade === 'emergencia' && a.prioridade !== 'emergencia') return 1;
                // Depois urgentes
                if (a.prioridade === 'urgente' && b.prioridade === 'eletiva') return -1;
                if (b.prioridade === 'urgente' && a.prioridade === 'eletiva') return 1;
                // Por último, por data de criação
                return b.criadaEm.getTime() - a.criadaEm.getTime();
              })
              .map((solicitacao) => {
                const paciente = baseCadastro.buscarPaciente(solicitacao.pacienteId);
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              ...
                              {solicitacao.especialidadeOrigem} → {solicitacao.especialidadeDestino}
                            </p>
                            
                            <p className="text-sm">
                              <strong>Procedimento:</strong> {solicitacao.procedimentoSolicitado}
                            </p>
                            
                            <p className="text-sm">
                              <strong>CID:</strong> {solicitacao.cid} • 
                              <strong>Unidade:</strong> {solicitacao.unidadeOrigem}
                            </p>
                            
                            <p className="text-xs text-muted-foreground">
                              Solicitante: {solicitacao.profissionalSolicitante} • 
                              Tempo de espera: {calcularTempoEspera(solicitacao.criadaEm)} •
                              {solicitacao.dataLimite && ` Prazo: ${solicitacao.dataLimite.toLocaleDateString('pt-BR')}`}
                            </p>
                          </div>
                        </div>
              ...
              <div className="space-y-4">
                {solicitacoes
                  ...
                  .map((solicitacao) => {
                    ...
                    return (
                      <Card key={solicitacao.id} className="border-l-4 border-l-red-500">
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-red-600">
                                ...
                                Tempo crítico: {calcularTempoEspera(solicitacao.criadaEm)}
                              </p>
                            </div>
