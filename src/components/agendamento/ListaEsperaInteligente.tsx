...
export const ListaEsperaInteligente = () => {
  ...
  const especialidades = Array.from(new Set(listaEspera.map(item => item.especialidade)));

  const listaFiltrada = especialidadeSelecionada 
    ? listaEspera.filter(item => item.especialidade === especialidadeSelecionada)
    : listaEspera;
  ...
  const processarListaAutomaticamente = async () => {
    if (!especialidadeSelecionada) {
      toast({
        ...
        description: "Escolha uma especialidade para processar a lista de espera",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Agendamentos automáticos foram criados conforme disponibilidade",
      });
      ...
      toast({
        ...
        description: "Não foi possível processar a lista de espera automaticamente",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie a lista de espera com priorização automática e inteligência artificial
          </p>
        </div>
        <Button onClick={processarListaAutomaticamente} disabled={!especialidadeSelecionada}>
      ...
      <Tabs defaultValue="por-especialidade" className="w-full">
        ...
        <TabsContent value="por-especialidade" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <Button
                  variant={especialidadeSelecionada === "" ? "default" : "outline"}
                ...
                {especialidades.map((esp) => {
                  const count = listaEspera.filter(item => item.especialidade === esp).length;
                  return (
                    <Button
                      key={esp}
                      variant={especialidadeSelecionada === esp ? "default" : "outline"}
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Lista de Espera - {especialidadeSelecionada || "Todas as Especialidades"}
              </CardTitle>
              <CardDescription>
                {listaFiltrada.length} pacientes na lista • Ordenado por prioridade
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  SUS: {paciente.cartaoSus} • CPF: {paciente.cpf}
                                </p>
                                <div className="flex items-center gap-2 mt-1">
                                  <span className="text-xs">Especialidade: {item.especialidade}</span>
                                  {item.unidadePreferida && (
                                    <span className="text-xs">• Unidade preferida: {item.unidadePreferida}</span>
                                  )}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                              <div className="text-right">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  ...
                                  {calcularTempoEspera(item.dataInclusao)} na lista
                                </p>
                                <p className="text-xs text-muted-foreground">
                              ...
                              <div className="flex flex-col gap-1">
                                <div className="flex gap-1">
                                  <Button 
                                    size="sm" 
                                    variant="outline"
                                    onClick={() => alterarPrioridade(item.id, item.prioridade - 1)}
                                ...
                                <div className="flex gap-1">
                                  ...
                                  <Button 
                                    size="sm" 
                                    variant="outline"
                                    onClick={() => marcarComoDesistente(item.id)}
                                  >
                ...
                {listaFiltrada.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    Nenhum paciente na lista de espera para os filtros selecionados
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <TabsContent value="prioritarios">
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {listaEspera
                  .filter(item => item.prioridade <= 3 && item.statusAtual === 'aguardando')
                  .sort((a, b) => a.prioridade - b.prioridade)
                  .map((item) => {
                    const paciente = pacientes.get(item.pacienteId);
                    if (!paciente) return null;
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {item.especialidade} • {calcularTempoEspera(item.dataInclusao)} na lista
                              </p>
                              <div className="flex items-center gap-2 mt-1">
                                {getPrioridadeBadge(item.prioridade)}
                                <span className="text-xs">Critérios: {item.criteriosPriorizacao.join(', ')}</span>
                              </div>
                            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Todas as especialidades</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Lista → Agendamento</p>
              </CardContent>
            </Card>
