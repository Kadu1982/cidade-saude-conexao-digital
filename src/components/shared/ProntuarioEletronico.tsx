...
export const ProntuarioEletronico: React.FC<ProntuarioEletronicoProps> = ({ tipo }) => {
  return (
        ...
        <CardContent className="pt-4">
          <div className="flex flex-col md:flex-row justify-between gap-2 md:items-center">
            <div>
              ...
              <p className="text-muted-foreground">{PACIENTE.idade} anos, {PACIENTE.sexo}</p>
            ...
            <div className="flex flex-wrap gap-1 mt-2 md:mt-0">
              {PACIENTE.condicoesEspeciais.map((condicao) => (
                <Badge key={condicao} variant="outline">{condicao}</Badge>
              ))}
            ...
            {PACIENTE.historicoConsultas.map((consulta, index) => (
              <div key={index} className="p-3">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-medium">{consulta.data} - {consulta.especialidade}</p>
                    <p className="text-sm text-muted-foreground">{consulta.medico}</p>
                  </div>
                </div>
                <div className="mt-2">
                  <p className="text-sm"><span className="font-medium">Diagnóstico:</span> {consulta.diagnostico}</p>
                  <p className="text-sm"><span className="font-medium">Prescrição:</span> {consulta.prescricao}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border rounded-md divide-y">
            {PACIENTE.exames.map((exame, index) => (
              <div key={index} className="p-3">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-medium">{exame.data} - {exame.tipo}</p>
                    <p className="text-sm">
                      Resultado: 
                      <span className={exame.resultado === "Normal" ? "text-green-600" : "text-amber-600"}>
                        {" "}{exame.resultado}
                      </span>
                    </p>
                  </div>
            ...
            <ul className="space-y-2">
              <li className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">1 comprimido ao dia</p>
                </div>
                <Badge>Em uso</Badge>
              </li>
              <li className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">1 comprimido após almoço e jantar</p>
                </div>
                <Badge>Em uso</Badge>
              </li>
              <li className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">1 comprimido a cada 8 horas por 7 dias</p>
                </div>
