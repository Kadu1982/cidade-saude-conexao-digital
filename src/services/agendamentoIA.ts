...
import { Paciente, Agendamento, ListaEspera, Agenda, baseCadastro } from './baseCadastro';
...
export class AgendamentoIAService {
  ...
  preverProbabilidadeFalta(paciente: Paciente, data: Date, hora: string): number {
    const historico = baseCadastro.buscarAgendamentosPorPaciente(paciente.id);
  ...
  sugerirMelhorHorario(paciente: Paciente, horariosDisponiveis: string[]): string {
    const historico = baseCadastro.buscarAgendamentosPorPaciente(paciente.id);
  ...
  async reagendarAutomatico(agendamentoCancelado: Agendamento): Promise<string | null> {
    const paciente = baseCadastro.buscarPaciente(agendamentoCancelado.pacienteId);
    ...
    const melhorHorario = this.sugerirMelhorHorario(paciente, horariosDisponiveis);
  ...
  analisarDemandaEspecialidade(especialidade: string, dias: number = 30): AnaliseAgendamento {
    const dataInicio = new Date();
    dataInicio.setDate(dataInicio.getDate() - dias);

    const agendamentos = Array.from(baseCadastro.buscarDadosCompletos().agendamentos)
      .filter(a => a.criadoEm >= dataInicio);

    const agendas = Array.from(baseCadastro.buscarDadosCompletos().agendas)
      .filter(a => a.especialidade === especialidade);

    // Calcular ocupação média
    const totalSlots = agendas.reduce((sum, a) => sum + a.cotaTotal, 0);
    const slotsUtilizados = agendas.reduce((sum, a) => sum + a.cotaUtilizada, 0);
    const ocupacaoMedia = totalSlots > 0 ? slotsUtilizados / totalSlots : 0;

    // Analisar horários com maior falta
    const agendamentosPorHora: { [hora: string]: { total: number; faltas: number } } = {};
    ...
    if (ocupacaoMedia > 0.9) {
      sugestoesMelhoria.push('Considerar aumentar a oferta de horários para esta especialidade');
    }
    
    if (horariosComMaiorFalta.length > 0) {
      sugestoesMelhoria.push(`Implementar lembretes extras para horários ${horariosComMaiorFalta.slice(0, 2).join(', ')}`);
    }
    
    if (ocupacaoMedia < 0.6) {
      sugestoesMelhoria.push('Avaliar redimensionamento da agenda ou estratégias de divulgação');
    }
  ...
  processarListaEspera(especialidade: string): void {
    const listaEspera = baseCadastro.buscarListaEsperaPorEspecialidade(especialidade);
    const agendasDisponiveis = baseCadastro.buscarAgendasDisponiveis(especialidade, new Date());
    ...
    for (const agenda of agendasDisponiveis) {
      ...
      for (const horario of horariosDisponiveis) {
        ...
        const paciente = baseCadastro.buscarPaciente(proximoPaciente.pacienteId);
        if (paciente) {
          const novoAgendamento: Agendamento = {
            id: `agend_${Date.now()}`,
            pacienteId: paciente.id,
